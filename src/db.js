...
CREATE INDEX IF NOT EXISTS idx_trades_user_dt ON trades(user_id, datetime);
CREATE INDEX IF NOT EXISTS idx_trades_user_symbol ON trades(user_id, symbol_id);
CREATE INDEX IF NOT EXISTS idx_symbols_user ON symbols(user_id);
CREATE INDEX IF NOT EXISTS idx_alert_logs_user ON alert_logs(user_id, handled);
...
function seedDefaultAccount(userId) {
  try {
    const existing = db.prepare('SELECT COUNT(*) AS cnt FROM accounts WHERE user_id = ?').get(userId);
    if (existing && existing.cnt > 0) return 0;
    db.prepare('INSERT INTO accounts (user_id, name, broker, type, created_at) VALUES (?, ?, ?, ?, ?)')
      .run(userId, '默认账户', '', 'mixed', Date.now());
    return 1;
  } catch (e) {
    console.error('[seed] 初始化默认账户失败：', e.message);
    return 0;
  }
}
...
(function seedDefaultUser() {
  try {
    ...
    seedDefaultAccount(info.lastInsertRowid);
    seedDefaultSymbols(info.lastInsertRowid);
...
module.exports.seedDefaultAccount = seedDefaultAccount;
