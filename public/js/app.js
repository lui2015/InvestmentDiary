...
(function () {
  ...
  const labelCat = (c) => ({ stock: '股票', fund: '基金', future: '期货' }[c] || c);
    ...
    root.innerHTML = `
      <div style="max-width:380px;margin:6vh auto">
        <div class="card">
          ...
          <p class="muted mb">记录你的股票 / 基金 / 期货投资</p>
  ...
  async function renderPositions(root) {
    ...
    const positions = ov.data.positions.filter(p => p.qty > 0);
    root.innerHTML = `
  ...
  async function renderAlerts(root) {
    ...
    $('#btnCheck').onclick = async () => { const r = await guard(Api.checkAlerts()); toast(r.code === 0 ? (r.data.length ? '触发 ' + r.data.length + ' 条提醒' : '未触发新提醒') : '检查失败'); renderAlerts(root); };
  }
      ...
      <div class="grid">
        ${list.data.map(r => {
          ...
          return `<div class="card">
            ...
            ${tpl.data.fields.map(f => c[f.key] ? `<div class="mb"><div class="muted" style="font-size:12px">${f.label}</div><div>${esc(c[f.key])}</div></div>` : '').join('')}
  ...
  function bindGlobal() {
    ...
    $('.modal-mask').addEventListener('click', e => { if (e.target.classList.contains('modal-mask')) e.target.hidden = true; });
  }
